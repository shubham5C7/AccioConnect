  ...
  import {getProfilePicture, getUserFullName } from '../utils/storage'
  ...
  const profilePic = getProfilePicture(); // returns user's profile picture OR DEFAULT_AVATAR
  const name = getUserFullName();  // returns full name OR 'User'

    // Calling the Api of Create post
  const handleCreatePost = async () => {
    // Validate based on mode
    const isContentEmpty = mode === "blog" 
      ? !postData.content.trim() 
      : !postData.content; // For images, just check if URL exists
    ...
    // Clear HTML tags from images/videos URLs
        let cleanContent =postData.content;
        if(postData.contentType === 'image' || postData.contentType === "video"){
          cleanContent=postData.content.replace(/<[^>]*>/g,'').trim();
        } 
  ...
  const payload = {
    ...
    likesEnabled: postData.likesEnabled,
  };
    ...
    try {
      ...
      setIsReferral(false);
      setMode("blog"); // Reset mode

      if (onClose) onClose();
    } catch (err) {
      console.error("Post creation failed:", err);
      // Consider showing error toast here
    }
  };
    const handleCancel = () => {
      setPostData({
